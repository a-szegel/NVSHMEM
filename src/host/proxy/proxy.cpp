...
inline int process_channel_put_signal(proxy_state_t *state, proxy_channel_t *ch, int *is_processed) {
    int pe, status = 0;
    base_request_t *base_req;
     ...
     *   - Memory used in psig operation has been updated before use (via flag)
     *   - Marking psig operation complete before operation has finished
     */
    ...
    ps_req_1 = (put_signal_request_1 *)WRAPPED_CHANNEL_BUF(
        state, ch, (ch->processed + 16));
    flag = COUNTER_TO_FLAG(state, (ch->processed + 16));
    while (*((volatile uint8_t *)&ps_req_1->flag) != flag)
        ;

    ps_req_2 = (put_signal_request_2 *)WRAPPED_CHANNEL_BUF(
        state, ch, (ch->processed + 24));
    flag = COUNTER_TO_FLAG(state, (ch->processed + 24));
    while (*((volatile uint8_t *)&ps_req_2->flag) != flag)
        ;

    ps_req_3 = (put_signal_request_3 *)WRAPPED_CHANNEL_BUF(
        state, ch, (ch->processed + 32));
    flag = COUNTER_TO_FLAG(state, (ch->processed + 32));
    while (*((volatile uint8_t *)&ps_req_3->flag) != flag)
        ;

    ps_req_4 = (put_signal_request_4 *)WRAPPED_CHANNEL_BUF(
        state, ch, (ch->processed + 40));
    ...
    size_remaining = (size_t)(((size_t)(ps_req_1->write_size_high) << 16) | (ps_req_1->write_size_low));
    while (size_remaining) {
        ...
        nvshmemi_get_local_mem_handle(&write_local_desc.handle, &local_chunk_size, lwrite_ptr, state->transport_id[pe]);
        nvshmemi_get_remote_mem_handle(&write_remote_desc, &remote_chunk_size, rwrite_ptr, pe, state->transport_id[pe]);
    ...
    rsig_offset = (uint64_t)(((uint64_t)(ps_req_3->rsigoffset_high) << 8) | (ps_req_3->rsigoffset_low));
    ...
    nvshmemi_get_remote_mem_handle(&sig_target_desc.remote_memdesc, NULL, rsig_ptr, pe, state->transport_id[pe]);
    ...
    status = tcurr->host_ops.psig(tcurr, pe, write_verb, remote_write_desc_vec, local_write_desc_vec, write_bytes_vec,
                                  sig_verb, &sig_target_desc, sig_bytes_desc, 1);
    if (unlikely(status)) {
