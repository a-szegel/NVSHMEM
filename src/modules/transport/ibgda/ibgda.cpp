...
struct ibgda_qp_control_structures {
...
struct ibgda_ep {
    ...
    struct ibgda_qp_control_structures qp_ctrl;
...
struct ibgda_device {
    ...
    struct {
        struct ibgda_qp_control_structures dci_ctrl;
        struct ibgda_ep **eps;
        int num_eps;
        int num_shared_eps;
...
typedef struct {
    ...
    // Cached CUDA information (computed once)
    CUdevice cached_gpu_device_id;
    int cached_warp_size;
    int cached_mtpb;
    int cached_mpc;
    ...
    bool support_half_av_seg;
    bool skip_cst;
    bool connect_endpoints_first_call;
    int last_device_index;  // For round-robin device selection
...
static void ibgda_get_device_qp(nvshmemt_ibgda_state_t *ibgda_state,
...
static int ibgda_parse_nic_handler_request(ibgda_nic_handler_t *out_loc, const char *str) {
    int status = 0;
    ibgda_nic_handler_t loc = IBGDA_NIC_HANDLER_AUTO;
    std::string req = str;
...
static int get_pci_path(int dev, char **pci_path, nvshmem_transport_t t) {
    ...
    struct ibgda_device *device = &(((struct ibgda_device *)ibgda_state->devices)[dev_id]);
    status = nvshmemt_ib_iface_get_mlx_path(
        device->common_device.dev, device->common_device.context, pci_path, &ftable, &mlx5dv_ftable,
        &(device->common_device.data_direct), ibgda_state->log_level);
...
int ibgda_dci_progress(nvshmem_transport_t t) {
    ...
    for (int j = 0; j < n_devs_selected; j++) {
        ...
        num_prod_idx_slots = device->dci.num_eps;
        prod_idx_cache = device->dci.dci_ctrl.prod_idx_cache;
        prod_idx_snapshot = device->dci.dci_ctrl.prod_idx_snapshot;
...
int ibgda_rc_progress(nvshmem_transport_t t) {
    nvshmemt_ibgda_state_t *ibgda_state = (nvshmemt_ibgda_state_t *)t->state;
    int n_devs_selected = ibgda_state->n_devs_selected;
    int n_pes = t->n_pes;
    struct mlx5_wqe_ctrl_seg ctrl_seg;
    for (int j = 0; j < n_devs_selected; j++) {
        ...
        int num_eps;
        uint64_t *prod_idx_snapshot;
        uint64_t *prod_idx_buffer;
        uint64_t *prod_idx_cache;

        dev_idx = ibgda_state->selected_dev_ids[j];
        device = (struct ibgda_device *)ibgda_state->devices + dev_idx;
        num_eps = device->rc.num_eps_per_pe * n_pes;
        for (int i = 0; i < num_eps; ++i) {
            ...
            prod_idx_buffer = (uint64_t *)ep->qp_ctrl.prod_idx_mobject->aligned.cpu_ptr;
            prod_idx_cache = ep->qp_ctrl.prod_idx_cache;
            prod_idx_snapshot = ep->qp_ctrl.prod_idx_snapshot;
            __be32 *dbrec;
            __be64 *bf;
...
int nvshmemt_ibgda_get_mem_handle(nvshmem_mem_handle_t *mem_handle, void *buf, size_t length,
    ...
    for (int i = 0; i < n_devs_selected; ++i) {
        ...
        INFO(ibgda_state->log_level, "[%d] IBGDA: device used %s, data_direct support: %d",
             transport->my_pe, device->common_device.dev->name, device->common_device.data_direct);
        status = nvshmemt_ib_common_reg_mem_handle(
            &ftable, &mlx5dv_ftable, device->common_device.pd, dev_handle, buf, length, local_only,
            ibgda_state->dmabuf_support_for_data_buffers, ibgda_cuda_syms, ibgda_state->log_level,
            ibgda_state->options->IB_ENABLE_RELAXED_ORDERING, device->common_device.data_direct,
            alias_va_ptr);
        NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
...
/* =============================================================================
 * Memory handle management end
 * ============================================================================= */

/* =============================================================================
 * Memory object management start
 * ============================================================================= */
...
#endif
        /* We only use this in the case that we have CPU proxy. When extending support to SM-Side
         * code, we will need to add an explicit check for mmio support in the cuda driver.
         */
        if (use_nc_uar) {
        uar = mlx5dv_devx_alloc_uar(context, MLX5DV_UAR_ALLOC_TYPE_NC);
        if (uar) uar_reg_size = IBGDA_MLX5_NC_UAR_SIZE;
    }
...
out:
    if (status) {
        ibgda_unmap_and_free_qp_uar(uar_mobject);
    }
    return status;
}

/* =============================================================================
 * Memory object management end
 * ============================================================================= */
...
static void ibgda_destroy_cq_mobjects(struct ibgda_cq *gcq) {
    if (gcq->dbr_mobject) {
        if (gcq->dbr_mobject->has_nic_mapping) ibgda_mobject_nic_unmap(gcq->dbr_mobject);
        ibgda_nic_control_free(gcq->dbr_mobject);
    }

    if (gcq->cq_mobject) {
        if (gcq->cq_mobject->has_nic_mapping) ibgda_mobject_nic_unmap(gcq->cq_mobject);
        ibgda_nic_control_free(gcq->cq_mobject);
    }
}

static int ibgda_create_cq_mobjects(nvshmemt_ibgda_state_t *ibgda_state, struct ibgda_cq *gcq,
                                    struct ibgda_device *device) {
    int status = 0;
    ...
    status = ibgda_nic_control_alloc(&gcq->cq_mobject, cq_buf_size, IBGDA_GPAGE_SIZE);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out, "cannot allocate cq buf.\n");

    status = cudaMemset(gcq->cq_mobject->base.gpu_ptr, 0xff, gcq->cq_mobject->base.size);
    NVSHMEMI_NE_ERROR_JMP(status, cudaSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "cudaMemset failed.\n");

    status = ibgda_mobject_nic_map(gcq->cq_mobject, context, IBV_ACCESS_LOCAL_WRITE,
...
out:
    if (status) {
        ibgda_destroy_cq_mobjects(gcq);
    }
    return status;
}

static int ibgda_create_cq(nvshmemt_ibgda_state_t *ibgda_state, struct ibgda_cq **pgcq,
                           struct ibgda_device *device) {
    int status = 0;
    ...
    status = ibgda_create_cq_mobjects(ibgda_state, gcq, device);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                          "ibgda_create_cq_mobjects failed.\n");

    cq_mobject = gcq->cq_mobject;
    dbr_mobject = gcq->dbr_mobject;
    ...
    DEVX_SET64(cqc, cq_context, dbr_addr, 0);  // DBR offset
...
static void ibgda_get_device_cq(nvshmemi_ibgda_device_cq_t *dev_cq, const struct ibgda_cq *gcq) {
    dev_cq->cqn = gcq->cqn;
    dev_cq->ncqes = gcq->num_cqe;

    assert(gcq->cq_mobject->has_gpu_mapping);
    dev_cq->cqe = (void *)((uintptr_t)gcq->cq_mobject->aligned.gpu_ptr);

    assert(gcq->dbr_mobject->has_gpu_mapping);
...
/* =============================================================================
 * QPair state transition functions start
 * ============================================================================= */
...
/* =============================================================================
 * QPair state transition functions end
 * ============================================================================= */

/* =============================================================================
 * Common qpair control structure management functionsstart
 * ============================================================================= */

static int ibgda_destroy_internal_buffer(struct ibgda_internal_buffer *internal_buf,
                                         nvshmemt_ibgda_state_t *ibgda_state) {
    int status = 0;

    if (internal_buf->mem_handle) {
        nvshmemt_ib_common_release_mem_handle(
            &ftable, (nvshmem_mem_handle_t *)internal_buf->mem_handle, ibgda_state->log_level);
        free(internal_buf->mem_handle);
    }

    if (internal_buf->mem_object) {
        ibgda_gpu_mem_free(internal_buf->mem_object);
    }
...
static int ibgda_create_internal_buffer(struct ibgda_internal_buffer *internal_buf,
                                        nvshmemt_ibgda_state_t *ibgda_state,
                                        struct ibgda_device *device, size_t size) {
    int status = 0;
    struct ibgda_mem_object *internal_buf_mobject = NULL;
    struct nvshmemt_ib_common_mem_handle *internal_buf_mhandle = NULL;

    status = ibgda_gpu_mem_alloc(&internal_buf_mobject, size, IBGDA_GPAGE_SIZE, false);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
    ...
    status = nvshmemt_ib_common_reg_mem_handle(
        &ftable, &mlx5dv_ftable, device->common_device.pd,
        (nvshmem_mem_handle_t *)internal_buf_mhandle, (void *)internal_buf_mobject->aligned.gpu_ptr,
        internal_buf_mobject->aligned.size, false, ibgda_state->dmabuf_support_for_data_buffers,
        ibgda_cuda_syms, ibgda_state->log_level, ibgda_state->options->IB_ENABLE_RELAXED_ORDERING,
...
out:
    if (status) {
        ibgda_destroy_internal_buffer(internal_buf, ibgda_state);
    }
    return status;
}
...
static void ibgda_destroy_qp_mobjects(struct ibgda_qp_control_structures *qp_ctrl) {
    ...
    if (qp_ctrl->prod_idx_cache) free(qp_ctrl->prod_idx_cache);

    if (qp_ctrl->prod_idx_snapshot) free(qp_ctrl->prod_idx_snapshot);

    if (qp_ctrl->dbr_mobject) {
        if (qp_ctrl->dbr_mobject->has_nic_mapping) ibgda_mobject_nic_unmap(qp_ctrl->dbr_mobject);
        if (ibgda_nic_handler == IBGDA_NIC_HANDLER_GPU)
            ibgda_nic_control_free(qp_ctrl->dbr_mobject);
        else
            ibgda_host_mem_free(qp_ctrl->dbr_mobject);
    }

    if (qp_ctrl->wq_mobject) {
        if (qp_ctrl->wq_mobject->has_nic_mapping) ibgda_mobject_nic_unmap(qp_ctrl->wq_mobject);
        ibgda_nic_control_free(qp_ctrl->wq_mobject);
    }
}

static int ibgda_create_qp_mobjects(struct ibgda_qp_control_structures *qp_ctrl,
                                    nvshmemt_ibgda_state_t *ibgda_state,
                                    struct ibgda_device *device, int num_eps) {
    int status = 0;

    struct ibv_context *context = device->common_device.context;

    size_t wq_buf_size = device->qp_shared_object.wq_buf_size_per_qp * num_eps;
    struct ibgda_mem_object *wq_mobject = NULL;

    size_t dbr_buf_size = IBGDA_DBRSIZE * num_eps;
    ...
    if (ibgda_nic_handler == IBGDA_NIC_HANDLER_CPU_GDRCOPY ||
        ...
        if (ibgda_nic_handler == IBGDA_NIC_HANDLER_CPU_GDRCOPY) {
            ...
            status = ibgda_host_mem_alloc(&prod_idx_mobject, sizeof(uint64_t) * num_eps,
                                          IBGDA_GPAGE_SIZE, true);
            NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                                  "cannot allocate prod_idx_mobject.\n");
        }
    ...
    qp_ctrl->wq_mobject = wq_mobject;
    qp_ctrl->dbr_mobject = dbr_mobject;
    qp_ctrl->prod_idx_mobject = prod_idx_mobject;
    qp_ctrl->prod_idx_cache = prod_idx_cache;
    qp_ctrl->prod_idx_snapshot = prod_idx_snapshot;

out:
    if (status) {
        ibgda_destroy_qp_mobjects(qp_ctrl);
    }
    return status;
}
...
static int ibgda_rc_create_internal_buffer(struct ibgda_ep *ep, nvshmemt_ibgda_state_t *ibgda_state,
                                           struct ibgda_device *device) {
    int status = 0;
    size_t size =
        NVSHMEMI_IBGDA_IBUF_SLOT_SIZE * (ibgda_num_fetch_slots_per_rc + IBGDA_IBUF_RESERVED_SLOTS);

    status = ibgda_create_internal_buffer(&ep->qp_ctrl.internal_buf, ibgda_state, device, size);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
 ...
 */
static int ibgda_create_qp(nvshmemt_ibgda_state_t *ibgda_state, struct ibgda_ep **ep_ptr,
                           struct ibgda_device *device, int portid, uint32_t qp_idx,
                           nvshmemi_ibgda_device_qp_type_t qp_type) {
    struct ibv_pd *pd = device->common_device.pd;
    struct ibv_context *context = pd->context;
    ...
    if (qp_type == NVSHMEMI_IBGDA_DEVICE_QP_TYPE_RC) {
        status = ibgda_rc_create_internal_buffer(ep, ibgda_state, device);
        NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                              "ibgda_rc_create_internal_buffer failed.\n");
        status = ibgda_create_qp_mobjects(&ep->qp_ctrl, ibgda_state, device, 1);
        NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                              "ibgda_create_qp_mobjects failed.\n");
    } else {
        ep = (struct ibgda_ep *)calloc(1, sizeof(struct ibgda_ep));
        NVSHMEMI_NULL_ERROR_JMP(ep, status, NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                                "Unable to allocate mem for ep.\n");
        ep->qp_ctrl.wq_mobject = device->dci.dci_ctrl.wq_mobject;
        ep->qp_ctrl.wq_offset = device->dci.cur_wq_off;

        ep->qp_ctrl.dbr_mobject = device->dci.dci_ctrl.dbr_mobject;
        ep->qp_ctrl.dbr_offset = device->dci.cur_dbr_off;

        ep->qp_ctrl.prod_idx_mobject = device->dci.dci_ctrl.prod_idx_mobject;
        ep->qp_ctrl.prod_idx_cache = device->dci.dci_ctrl.prod_idx_cache;
        ep->qp_ctrl.prod_idx_snapshot = device->dci.dci_ctrl.prod_idx_snapshot;

        device->dci.cur_wq_off += device->qp_shared_object.wq_buf_size_per_qp;
        device->dci.cur_dbr_off += IBGDA_DBRSIZE;
    }

    DEVX_SET(create_qp_in, cmd_in, opcode, MLX5_CMD_OP_CREATE_QP);
    DEVX_SET(create_qp_in, cmd_in, wq_umem_id, ep->qp_ctrl.wq_mobject->umem->umem_id);  // WQ buffer
    DEVX_SET64(create_qp_in, cmd_in, wq_umem_offset, ep->qp_ctrl.wq_offset);
    ...
    DEVX_SET64(
        qpc, qp_context, dbr_addr,
        ep->qp_ctrl
            .dbr_offset);  // Offset of dbr_umem_id (behavior changed because of dbr_umem_valid)
    DEVX_SET(qpc, qp_context, dbr_umem_id, ep->qp_ctrl.dbr_mobject->umem->umem_id);  // DBR buffer
...
static int ibgda_get_rc_handle(struct ibgda_rc_handle *rc_handle, const struct ibgda_ep *ep,
                               const struct ibgda_device *device) {
    const struct ibv_port_attr *port_attr = &device->common_device.port_attr[ep->portid - 1];
...
static int ibgda_create_dct_shared_objects(nvshmemt_ibgda_state_t *ibgda_state,
    ...
    pd_init_attr.pd = device->common_device.pd;
...
static int ibgda_get_cuda_device_attributes(CUdevice *gpu_device_id, int *warp_size, int *mtpb,
                                            int *mpc) {
    int status = 0;
...
static int ibgda_validate_dct_endpoints(nvshmemt_ibgda_state_t *ibgda_state,
                                        struct ibgda_device *device, int num_selected_devs) {
    int status = 0;
    int num_dct_eps = ibgda_state->options->IBGDA_NUM_DCT;

    /* DCT endpoint validation start */
    if ((num_dct_eps * num_selected_devs) < 2) {
        NVSHMEMI_WARN_PRINT("NVSHMEM_IBGDA_NUM_DCT < 2 and may impact performance.");
    }

    /* Set device DCT information */
    device->dct.num_eps = num_dct_eps;
    /* DCT endpoint validation end */

    return status;
}

static int ibgda_allocate_dct_structures(struct ibgda_device *device, int n_pes) {
    int status = 0;

    /* allocate DCT device structs start */
    ...
    /* allocate DCT device structs end */

out:
    return status;
}

static int ibgda_setup_dct_endpoints(nvshmemt_ibgda_state_t *ibgda_state,
                                     struct ibgda_device *device, int portid,
                                     nvshmem_transport_t t) {
    int status = 0;
    struct ibgda_dct_handle *local_dct_handles = NULL;

    /* allocate local DCT handles start */
    local_dct_handles =
    ...
    /* allocate local DCT handles end */
...
static int ibgda_populate_dct_gpu_data(nvshmemt_ibgda_state_t *ibgda_state, nvshmem_transport_t t,
                                       nvshmemi_ibgda_device_dct_t *dct_h, int num_dct_handles) {
    int status = 0;
    int n_devs_selected = ibgda_state->n_devs_selected;

    /* Get and store DCT information start */
    ...
    /* Get and store DCT information end */

    return status;
}

static int ibgda_setup_dct_gpu_state(nvshmemt_ibgda_state_t *ibgda_state, nvshmem_transport_t t,
                                     int *num_dct_handles, int *num_dct_non_cache_handles,
                                     nvshmemi_ibgda_device_dct_t **dct_h,
                                     nvshmemi_ibgda_device_dct_t **dct_d) {
    int status = 0;
    int n_pes = t->n_pes;
    int n_devs_selected = ibgda_state->n_devs_selected;

    /* Calculate DCT buffer sizes start */
    *num_dct_handles = 0;
    for (int j = 0; j < n_devs_selected; j++) {
        int dev_idx = ibgda_state->selected_dev_ids[j];
        struct ibgda_device *device = (struct ibgda_device *)ibgda_state->devices + dev_idx;
    ...
    if (*num_dct_handles > NVSHMEMI_IBGDA_MAX_CONST_DCTS) {
        ...
        *num_dct_non_cache_handles = 0;
    }
    /* Calculate DCT buffer sizes end */

    /* Allocate DCT host memory start */
    if (*dct_h == NULL) {
        *dct_h = (nvshmemi_ibgda_device_dct_t *)calloc(*num_dct_handles, sizeof(**dct_h));
        NVSHMEMI_NULL_ERROR_JMP(*dct_h, status, NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                                "dct calloc err.");
        /* Allocate DCT host memory end */
        /* Allocate DCT device memory start */
    ...
    /* Allocate DCT device memory end */

out:
    return status;
}

static int ibgda_copy_dct_gpu_data(nvshmemt_ibgda_state_t *ibgda_state,
                                   nvshmemi_ibgda_device_dct_t *dct_h,
                                   nvshmemi_ibgda_device_dct_t *dct_d, int num_dct_handles,
                                   int num_dct_non_cache_handles,
...
/* =============================================================================
 * DCI HELPER FUNCTIONS
 * ============================================================================= */

static int ibgda_calculate_dci_endpoints(nvshmemt_ibgda_state_t *ibgda_state,
                                         struct ibgda_device *device, int n_pes, int warp_size,
                                         int mpc) {
    int status = 0;
    int num_dci_eps = ibgda_state->options->IBGDA_NUM_DCI;
    int num_shared_dci_eps = ibgda_state->options->IBGDA_NUM_SHARED_DCI;
    ...
    INFO(ibgda_state->log_level, "IBGDA_DCI_MAP_BY is set to %s.",
         ibgda_state->options->IBGDA_DCI_MAP_BY);
    /* Parse DCI mapping type end */

    /* DCI endpoint calculation and validation start */
    ...
    if (num_dci_eps <= 0) {
        switch (dc_map_type) {
            ...
            case NVSHMEMI_IBGDA_DEVICE_QP_MAP_TYPE_DCT:
                num_dci_eps = device->dct.num_eps * n_pes;
                break;
            default:
                NVSHMEMI_ERROR_JMP(status, NVSHMEMX_ERROR_INVALID_VALUE, out,
    ...
    /* Validate DCI configuration */
    if (num_shared_dci_eps > num_dci_eps) {
        NVSHMEMI_ERROR_JMP(status, NVSHMEMX_ERROR_INVALID_VALUE, out,
                           "IBGDA_NUM_SHARED_DCI > IBGDA_NUM_DCI.");
    }

    /* Set device DCI information */
    device->dci.map_by = dc_map_type;
    device->dci.num_eps = num_dci_eps;
    device->dci.num_shared_eps = num_shared_dci_eps;
    /* DCI endpoint calculation and validation end */

out:
    return status;
}

static int ibgda_allocate_dci_structures(struct ibgda_device *device) {
    int status = 0;

    /* allocate DCI device structs start */
    ...
    /* allocate DCI device structs end */

out:
    return status;
}

static int ibgda_setup_dci_endpoints(nvshmemt_ibgda_state_t *ibgda_state,
                                     struct ibgda_device *device, int portid) {
    int status = 0;
    ...
    for (int i = 0; i < device->dci.num_eps; ++i) {
        status = ibgda_create_qp(ibgda_state, &device->dci.eps[i], device, portid, i,
...
static int ibgda_destroy_dci_internal_buffer(nvshmemt_ibgda_state_t *ibgda_state,
                                             struct ibgda_device *device) {
    int status = 0;
    status = ibgda_destroy_internal_buffer(&device->dci.dci_ctrl.internal_buf, ibgda_state);
    return status;
}

static int ibgda_create_dci_internal_buffer(nvshmemt_ibgda_state_t *ibgda_state,
                                            struct ibgda_device *device) {
    int status = 0;

    size_t size_per_dci =
        NVSHMEMI_IBGDA_IBUF_SLOT_SIZE * (ibgda_num_fetch_slots_per_dci + IBGDA_IBUF_RESERVED_SLOTS);
    size_t buf_size = (size_per_dci * device->dci.num_eps);

    status = ibgda_create_internal_buffer(&device->dci.dci_ctrl.internal_buf, ibgda_state, device,
                                          buf_size);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                          "ibgda_create_internal_dci_buffer failed.\n");

out:
    if (status) {
        ibgda_destroy_internal_buffer(&device->dci.dci_ctrl.internal_buf, ibgda_state);
    }
    return status;
}

static int ibgda_destroy_dci_shared_objects(nvshmemt_ibgda_state_t *ibgda_state,
                                            struct ibgda_device *device) {
    int status = 0;
    status = ibgda_destroy_dci_internal_buffer(ibgda_state, device);

    ibgda_destroy_qp_mobjects(&device->dci.dci_ctrl);
    return status;
}

static int ibgda_create_dci_shared_objects(nvshmemt_ibgda_state_t *ibgda_state,
                                           struct ibgda_device *device) {
    int status = 0;

    status = ibgda_create_dci_internal_buffer(ibgda_state, device);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                          "ibgda_create_dci_internal_buffer failed.\n");

    status =
        ibgda_create_qp_mobjects(&device->dci.dci_ctrl, ibgda_state, device, device->dci.num_eps);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                          "ibgda_create_qp_mobjects failed.\n");
...
static int ibgda_populate_dci_gpu_data(nvshmemt_ibgda_state_t *ibgda_state, nvshmem_transport_t t,
                                       nvshmemi_ibgda_device_qp_t *dci_h,
    ...
    /* Get and store DCI information start */
    for (int i = 0; i < num_dci_handles / n_devs_selected; ++i) {
        for (int j = 0; j < n_devs_selected; j++) {
            ...
            ibgda_get_device_qp(ibgda_state, &dci_h[arr_idx], device, device->dci.eps[i],
                                i - device->dci.num_eps, j);
            dci_h[arr_idx].tx_wq.cq = &cq_d[cq_idx];
            TRACE(ibgda_state->log_level, "Populating DCI at qp_idx #%d, cq_idx #%d", arr_idx,
                  cq_idx);
...
static int ibgda_setup_dci_gpu_state(nvshmemt_ibgda_state_t *ibgda_state, nvshmem_transport_t t,
                                     int *num_dci_handles, int *num_shared_dci_handles,
                                     nvshmemi_ibgda_device_qp_t **dci_h,
                                     nvshmemi_ibgda_device_qp_t **dci_d) {
    int status = 0;
    int n_devs_selected = ibgda_state->n_devs_selected;

    /* Calculate DCI buffer sizes start */
    *num_dci_handles = 0;
    *num_shared_dci_handles = 0;
    for (int j = 0; j < n_devs_selected; j++) {
        int dev_idx = ibgda_state->selected_dev_ids[j];
        struct ibgda_device *device = (struct ibgda_device *)ibgda_state->devices + dev_idx;
    ...
    /* Calculate DCI buffer sizes end */

    if (*dci_h == NULL) {
        /* Allocate DCI host memory start */
        *dci_h = (nvshmemi_ibgda_device_qp_t *)calloc(*num_dci_handles, sizeof(**dci_h));
        ...
        /* Allocate DCI host memory end */

        /* Allocate DCI device memory start */
        status = cudaMalloc(dci_d, *num_dci_handles * sizeof(**dci_d));
        NVSHMEMI_NE_ERROR_JMP(status, cudaSuccess, NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                              "dci cudaM err.");
        /* Allocate DCI device memory end */
    }
...
static int ibgda_copy_dci_gpu_data(nvshmemt_ibgda_state_t *ibgda_state,
                                   nvshmemi_ibgda_device_qp_t *dci_h,
                                   nvshmemi_ibgda_device_qp_t *dci_d, int num_dci_handles) {
    int status = 0;
...
/* =============================================================================
 * RC (Reliable Connection) HELPER FUNCTIONS
 * ============================================================================= */

static int ibgda_calculate_rc_endpoints(nvshmemt_ibgda_state_t *ibgda_state,
                                        struct ibgda_device *device, int warp_size) {
    int status = 0;
    int num_rc_eps_per_pe = ibgda_state->options->IBGDA_NUM_RC_PER_PE;
    nvshmemi_ibgda_device_qp_map_type_t rc_map_type;

    /* RC endpoint calculation and validation start */
    if (num_rc_eps_per_pe < 0) {
        return NVSHMEMX_SUCCESS;
    } else {
        /* Parse RC mapping type start */
        status = ibgda_parse_qp_map_by(&rc_map_type, ibgda_state->options->IBGDA_RC_MAP_BY);
        NVSHMEMI_NZ_ERROR_JMP(status, status, out, "IBGDA_RC_MAP_BY is not valid.");
        INFO(ibgda_state->log_level, "IBGDA_RC_MAP_BY is set to %s.",
             ibgda_state->options->IBGDA_RC_MAP_BY);
        /* Parse RC mapping type end */
    ...
    /* Set device RC information */
    device->rc.map_by = rc_map_type;
    /* RC endpoint calculation and validation end */

out:
    return status;
}

static int ibgda_allocate_rc_structures(nvshmem_transport_t t, struct ibgda_device *device,
                                        int num_rc_eps) {
    int status = 0;
    int n_pes = t->n_pes;
    ...
    /* allocate RC device structs end */
...
static int ibgda_setup_rc_endpoints(nvshmemt_ibgda_state_t *ibgda_state,
    ...
    struct ibgda_rc_handle *peer_ep_handles = NULL;
    ...
    INFO(ibgda_state->log_level, "Creating %d RC QPs", num_eps_per_pe);
    for (int i = 0; i < num_eps_per_pe; ++i) {
        for (int j = 0; j < n_pes; ++j) {
            ...
            TRACE(ibgda_state->log_level, "dst_pe: %d, mapped_i: %d, local_mapped_i: %d", dst_pe,
            ...
            NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                                  "ibgda_create_rc failed on RC #%d.", mapped_i);

            status = ibgda_get_rc_handle(&local_rc_handles[local_mapped_i],
    ...
    for (int i = 0; i < num_eps_per_pe; ++i) {
        for (int j = 0; j < n_pes; ++j) {
            ...
            int peer_handle_index = num_eps_per_pe * j + i;
            // No loopback to self
            if (j == mype) {
                continue;
            }
    ...
    device->rc.num_eps_per_pe += num_eps_per_pe;
    ibgda_state->cur_qp_index += num_rc_eps;
...
static int ibgda_copy_rc_gpu_data(nvshmemt_ibgda_state_t *ibgda_state,
                                  nvshmemi_ibgda_device_qp_t *rc_h,
                                  nvshmemi_ibgda_device_qp_t *rc_d, int num_rc_handles) {
    int status = 0;
...
static int ibgda_setup_rc_gpu_state(nvshmemt_ibgda_state_t *ibgda_state, nvshmem_transport_t t,
    ...
    int n_pes = t->n_pes;
    int n_devs_selected = ibgda_state->n_devs_selected;
    nvshmemi_ibgda_device_qp_t *rc_d_temp = NULL;
    /* Calculate RC buffer sizes start */
    *num_rc_handles = 0;
    for (int j = 0; j < n_devs_selected; j++) {
        int dev_idx = ibgda_state->selected_dev_ids[j];
        struct ibgda_device *device = (struct ibgda_device *)ibgda_state->devices + dev_idx;
    ...
    INFO(ibgda_state->log_level, "num_rc_handles: %d", *num_rc_handles);
    /* Calculate RC buffer sizes end */

    /* Allocate RC host memory start */
    if (*num_rc_handles > 0) {
        if (*rc_h == NULL) {
            TRACE(ibgda_state->log_level, "Allocating RC host memory");
            *rc_h = (nvshmemi_ibgda_device_qp_t *)calloc(*num_rc_handles, sizeof(**rc_h));
        } else {
            TRACE(ibgda_state->log_level, "Reallocating RC host memory old: %p", *rc_h);
    ...
    /* Allocate RC host memory end */

    /* Allocate RC device memory start */
    if (*num_rc_handles > 0) {
        if (*rc_d != NULL) {
            status = cudaMalloc(&rc_d_temp, *num_rc_handles * sizeof(**rc_d));
            NVSHMEMI_NE_ERROR_JMP(status, cudaSuccess, NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                                  "rc_d cudaM err.");
            cudaMemcpyAsync(rc_d_temp, *rc_d, ibgda_state->last_num_rcs * sizeof(**rc_d),
                            cudaMemcpyDeviceToDevice, ibgda_state->my_stream);
            cudaMemcpyAsync(*rc_h, *rc_d, ibgda_state->last_num_rcs * sizeof(**rc_h),
                            cudaMemcpyDeviceToHost, ibgda_state->my_stream);
...
static int ibgda_setup_cq_gpu_state(nvshmemt_ibgda_state_t *ibgda_state, nvshmem_transport_t t,
                                    int num_dci_handles, int *num_cq_handles,
                                    nvshmemi_ibgda_device_cq_t **cq_h,
                                    nvshmemi_ibgda_device_cq_t **cq_d) {
    int status = 0;
    int n_pes = t->n_pes;
    int n_devs_selected = ibgda_state->n_devs_selected;
    nvshmemi_ibgda_device_cq_t *cq_d_temp = NULL;
    /* Calculate CQ buffer sizes start */
    *num_cq_handles = 0;
    for (int j = 0; j < n_devs_selected; j++) {
        int dev_idx = ibgda_state->selected_dev_ids[j];
        struct ibgda_device *device = (struct ibgda_device *)ibgda_state->devices + dev_idx;
    ...
    for (int i = ibgda_state->last_num_cqs; i < *num_cq_handles; i++) {
        nvshmemi_init_ibgda_device_cq((*cq_h)[i]);
    }
    /* Allocate CQ host memory end */
    ...
    if (*cq_d != NULL) {
        ...
        cudaMemcpyAsync(cq_d_temp, *cq_d, ibgda_state->last_num_cqs * sizeof(**cq_d),
                        cudaMemcpyDeviceToDevice, ibgda_state->my_stream);
        cudaMemcpyAsync(*cq_h, *cq_d, ibgda_state->last_num_cqs * sizeof(**cq_d),
                        cudaMemcpyDeviceToHost, ibgda_state->my_stream);
        cudaStreamSynchronize(ibgda_state->my_stream);
        cudaFree(*cq_d);
        *cq_d = cq_d_temp;
    ...
    /* Allocate CQ device memory end */
out:
    return status;
}

static int ibgda_copy_cq_gpu_data(nvshmemt_ibgda_state_t *ibgda_state,
                                  nvshmemi_ibgda_device_cq_t *cq_h,
...
static int ibgda_post_gpu_device_state(
    ...
    TRACE(ibgda_state->log_level, "Posting device state DCI_d: %p, RC_d: %p, DCT_d: %p, CQ_d: %p",
          dci_d, rc_d, dct_d, cq_d);
...
static int ibgda_destroy_ep(struct ibgda_ep *ep) {
    ...
    if (ep->qp_type == NVSHMEMI_IBGDA_DEVICE_QP_TYPE_RC) {
        ibgda_destroy_qp_mobjects(&ep->qp_ctrl);
    }
...
static void ibgda_get_device_qp(nvshmemt_ibgda_state_t *ibgda_state,
    ...
    const struct ibgda_qp_control_structures *qp_ctrl = &ep->qp_ctrl;

    assert(ep->qp_type == NVSHMEMI_IBGDA_DEVICE_QP_TYPE_DCI ||
           ep->qp_type == NVSHMEMI_IBGDA_DEVICE_QP_TYPE_RC);

    assert(qp_ctrl->wq_mobject->has_gpu_mapping);

    if (dev_qp->qpn != NVSHMEMI_IBGDA_USCALAR_INVALID) {
        TRACE(ibgda_state->log_level, "Not re-populating RC at ep_index #%d, qp_idx #%d, qpn: %u",
              ep_idx, ep->user_index, dev_qp->qpn);
        return;
    }
    ...
    if (ibgda_nic_handler == IBGDA_NIC_HANDLER_GPU) {
        ...
        dev_qp->tx_wq.dbrec = (__be32 *)((uintptr_t)qp_ctrl->dbr_mobject->aligned.gpu_ptr +
                                         qp_ctrl->dbr_offset + sizeof(__be32));

        assert(ep->uar_mobject->has_gpu_mapping);
        dev_qp->tx_wq.bf = (void *)ep->uar_mobject->aligned.gpu_ptr;
    } else {
        if (ep->qp_type == NVSHMEMI_IBGDA_DEVICE_QP_TYPE_DCI) {
            dev_qp->tx_wq.bf = (void *)((uintptr_t)qp_ctrl->prod_idx_mobject->aligned.gpu_ptr +
                                        (device->dci.num_eps + ep_idx) * sizeof(uint64_t));
        } else {
            dev_qp->tx_wq.bf = (void *)((uintptr_t)qp_ctrl->prod_idx_mobject->aligned.gpu_ptr);
        }
    }
    ...
    if (ep->qp_type == NVSHMEMI_IBGDA_DEVICE_QP_TYPE_DCI) {
        dev_qp->ibuf.buf = (void *)(ibuf_dci_start + (size_per_dci * ep->user_index));
        dev_qp->ibuf.nslots = ibgda_num_fetch_slots_per_dci;
        dev_qp->ibuf.lkey = htobe32(device->dci.dci_ctrl.internal_buf.mem_handle->lkey);
        dev_qp->ibuf.rkey = htobe32(device->dci.dci_ctrl.internal_buf.mem_handle->rkey);
    } else if (ep->qp_type == NVSHMEMI_IBGDA_DEVICE_QP_TYPE_RC) {
        dev_qp->ibuf.buf = (void *)(ep->qp_ctrl.internal_buf.mem_object->aligned.gpu_ptr);
        dev_qp->ibuf.nslots = ibgda_num_fetch_slots_per_rc;
        dev_qp->ibuf.lkey = htobe32(ep->qp_ctrl.internal_buf.mem_handle->lkey);
        dev_qp->ibuf.rkey = htobe32(ep->qp_ctrl.internal_buf.mem_handle->rkey);
    }
...
static int ibgda_setup_gpu_state(nvshmem_transport_t t) {
    ...
    bool need_dct_setup = false;

    nvshmemi_ibgda_device_dct_t *dct_d = ibgda_device_state_h->globalmem.dcts;
    nvshmemi_ibgda_device_dct_t *dct_h = NULL;

    nvshmemi_ibgda_device_qp_t *dci_d = ibgda_device_state_h->globalmem.dcis;
    nvshmemi_ibgda_device_qp_t *dci_h = NULL;

    nvshmemi_ibgda_device_qp_t *rc_d = ibgda_device_state_h->globalmem.rcs;
    ...
    /* Setup DCT GPU state start */
    if (ibgda_state->device_state_cache->dct_is_setup) {
        dct_d = ibgda_device_state_h->globalmem.dcts;
        num_dct_handles = ibgda_device_state_h->ndcts_per_pe * n_devs_selected * t->n_pes;
    } else {
        need_dct_setup = true;
        status =
            ibgda_setup_dct_gpu_state(ibgda_state, t, &num_dct_handles, &num_dct_non_cache_handles,
                                      &ibgda_state->device_state_cache->dct_h, &dct_d);
        NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                              "ibgda_setup_dct_gpu_state failed.");
        ibgda_state->device_state_cache->dct_is_setup = true;
    }
    dct_h = ibgda_state->device_state_cache->dct_h;
    /* Setup DCT GPU state end */

    /* Setup DCI GPU state start */
    if (ibgda_state->device_state_cache->dci_h != NULL) {
        dci_d = ibgda_device_state_h->globalmem.dcis;
    }

    status = ibgda_setup_dci_gpu_state(ibgda_state, t, &num_dci_handles, &num_shared_dci_handles,
                                       &ibgda_state->device_state_cache->dci_h, &dci_d);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                          "ibgda_setup_dci_gpu_state failed.");
    dci_h = ibgda_state->device_state_cache->dci_h;
    /* Setup DCI GPU state end */

    /* Setup RC GPU state start */
    status = ibgda_setup_rc_gpu_state(ibgda_state, t, &num_rc_handles,
                                      &ibgda_state->device_state_cache->rc_h, &rc_d);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out, "ibgda_setup_rc_gpu_state failed.");
    /* Setup RC GPU state end */

    /* Setup CQ GPU state start */
    status = ibgda_setup_cq_gpu_state(ibgda_state, t, num_dci_handles, &num_cq_handles,
                                      &ibgda_state->device_state_cache->cq_h, &cq_d);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out, "ibgda_setup_cq_gpu_state failed.");
    /* Setup CQ GPU state end */

    /* Setup QP groups start only set for RC QPs */
    if (num_rc_handles == ibgda_state->options->IBGDA_NUM_RC_PER_PE * n_devs_selected * t->n_pes) {
        status = ibgda_setup_qp_groups_gpu_state(ibgda_state, t, num_dci_handles, num_rc_handles,
        ...
        num_qp_groups = ibgda_device_state_h->num_qp_groups;
        qp_group_switches_d = ibgda_device_state_h->globalmem.qp_group_switches;
    }
    /* Setup QP groups end */

    /* Calculate remaining constants start */
    for (int j = 0; j < n_devs_selected; j++) {
        int dev_idx = ibgda_state->selected_dev_ids[j];
        struct ibgda_device *device = (struct ibgda_device *)ibgda_state->devices + dev_idx;
    ...
    if (need_dct_setup) {
        status = ibgda_populate_dct_gpu_data(ibgda_state, t, dct_h, num_dct_handles);
        NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                              "ibgda_populate_dct_gpu_data failed.");
    }
    ...
    TRACE(ibgda_state->log_level, "Populated RC GPU data");

    TRACE(ibgda_state->log_level,
          "num_dci_handles: %d, num_dct_handles: %d, num_cq_handles: %d, num_rc_handles: %d",
          num_dci_handles, num_dct_handles, num_cq_handles, num_rc_handles);

    status = ibgda_copy_dct_gpu_data(ibgda_state, dct_h, dct_d, num_dct_handles,
                                     num_dct_non_cache_handles, ibgda_device_state_h);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out, "ibgda_copy_dct_gpu_data failed.");
    TRACE(ibgda_state->log_level, "Copied DCT GPU data");

    status = ibgda_copy_dci_gpu_data(ibgda_state, dci_h, dci_d, num_dci_handles);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out, "ibgda_copy_dci_gpu_data failed.");
    TRACE(ibgda_state->log_level, "Copied DCI GPU data");

    status = ibgda_copy_rc_gpu_data(ibgda_state, ibgda_state->device_state_cache->rc_h, rc_d,
                                    num_rc_handles);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out, "ibgda_copy_rc_gpu_data failed.");
    TRACE(ibgda_state->log_level, "Copied RC GPU data");

    status = ibgda_copy_cq_gpu_data(ibgda_state, ibgda_state->device_state_cache->cq_h, cq_d,
                                    num_cq_handles);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out, "ibgda_copy_cq_gpu_data failed.");
    TRACE(ibgda_state->log_level, "Copied CQ GPU data");
...
out:
    if (status) {
        if (dci_h) free(dci_h);
        if (dct_h) free(dct_h);
        if (ibgda_state->device_state_cache->cq_h) free(ibgda_state->device_state_cache->cq_h);
        if (ibgda_state->device_state_cache->rc_h) free(ibgda_state->device_state_cache->rc_h);
...
static int ibgda_connect_global_setup(nvshmemt_ibgda_state_t *ibgda_state, int num_selected_devs,
    ...
    status = ibgda_get_cuda_device_attributes(&ibgda_state->cached_gpu_device_id,
                                              &ibgda_state->cached_warp_size,
    ...
    for (int i = 0; i < num_selected_devs; i++) {
        ...
        ibgda_state->selected_dev_ids[i] = ibgda_state->dev_ids[selected_dev_ids[i]];
    }
...
static int ibgda_connect_device_calculations(nvshmem_transport_t t,
                                             nvshmemt_ibgda_state_t *ibgda_state,
                                             struct ibgda_device *device, int dev_idx, int n_pes) {
    int status = 0;

    // Calculate and validate endpoint counts for this device
    status = ibgda_validate_dct_endpoints(ibgda_state, device, ibgda_state->n_devs_selected);
    if (status) return status;

    status = ibgda_calculate_dci_endpoints(ibgda_state, device, n_pes,
                                           ibgda_state->cached_warp_size, ibgda_state->cached_mpc);
    if (status) return status;

    status = ibgda_calculate_rc_endpoints(ibgda_state, device, ibgda_state->cached_warp_size);
    if (status) return status;

    return status;
}

// Phase 3: Per-device resource creation (cached)
static int ibgda_connect_device_resources(nvshmemt_ibgda_state_t *ibgda_state,
                                          struct ibgda_device *device, int dev_idx, int portid,
                                          nvshmem_transport_t t) {
    int status = 0;
    int n_pes = t->n_pes;
    int num_rc_eps_per_pe = ibgda_state->options->IBGDA_NUM_RC_PER_PE;
    ...
    status = ibgda_allocate_dct_structures(device, n_pes);
    if (status) return status;

    status = ibgda_allocate_dci_structures(device);
    if (status) return status;

    status = ibgda_allocate_rc_structures(t, device, num_rc_eps_per_pe * n_pes);
    if (status) return status;
...
static int ibgda_connect_device_endpoints(nvshmemt_ibgda_state_t *ibgda_state,
    ...
    int status = 0;
    // Setup DCT endpoints
    status = ibgda_setup_dct_endpoints(ibgda_state, device, portid, t);
    if (status) return status;

    // Setup DCI endpoints
    status = ibgda_setup_dci_endpoints(ibgda_state, device, portid);
    if (status) return status;
    ...
    int n_pes = t->n_pes;
    if (ibgda_state->support_half_av_seg) {
        for (int i = 0; i < device->dct.num_eps * n_pes; ++i) {
            ibgda_state->support_half_av_seg &= device->dct.dct_handles[i].support_half_av_seg;
        }
    }

    ibgda_state->skip_cst &= (!ibgda_cst_is_required(device, ibgda_state->cached_gpu_device_id));
...
static void ibgda_initialize_cache_state(nvshmemt_ibgda_state_t *ibgda_state) {
    ibgda_state->connect_endpoints_first_call = true;
    ibgda_state->last_device_index = 0;
    ibgda_state->last_num_rcs = 0;
    ibgda_state->last_num_cqs = 0;
}

// Helper function for subsequent RC-only calls
static int ibgda_connect_rc_only(nvshmemt_ibgda_state_t *ibgda_state, nvshmem_transport_t t,
    ...
    int n_pes = t->n_pes;
    int num_qps_per_device = num_qps / ibgda_state->n_devs_selected;

    num_qps_per_device += num_qps % ibgda_state->n_devs_selected ? 1 : 0;

    struct ibgda_device *device;

    // Bail if IBGDA_NUM_RC_PER_PE < 1 (indicates DCI rather than RC)
    if (ibgda_state->options->IBGDA_NUM_RC_PER_PE < 1) {
        return NVSHMEMX_SUCCESS;  // No RC endpoints to create
    }

    // Allocate out_qp_indices array with num_qps entries
    assert(out_qp_indices != NULL);

    for (int i = 0; i < ibgda_state->n_devs_selected; i++) {
        dev_idx = ibgda_state->selected_dev_ids[i];
        device = (struct ibgda_device *)ibgda_state->devices + dev_idx;
    ...
    for (int i = 0; i < num_qps; i++) {
        ...
        portid = ibgda_state->port_ids[selected_dev_idx];
        device = (struct ibgda_device *)ibgda_state->devices + dev_idx;
...
int nvshmemt_ibgda_connect_endpoints(nvshmem_transport_t t, int *selected_dev_ids,
    ...
    // Check if this is a subsequent call (RC-only)
    ...
    for (int i = 0; i < num_selected_devs; i++) {
        ...
        struct ibgda_device *device = (struct ibgda_device *)ibgda_state->devices + dev_idx;
        int portid = ibgda_state->port_ids[selected_dev_ids[i]];

        // Only process if not already done for this device
        status = ibgda_connect_device_calculations(t, ibgda_state, device, dev_idx, n_pes);
        if (status) return status;

        status = ibgda_connect_device_resources(ibgda_state, device, dev_idx, portid, t);
        if (status) return status;

        status = ibgda_connect_device_endpoints(ibgda_state, device, portid, t);
        if (status) return status;
...
int nvshmemt_ibgda_finalize(nvshmem_transport_t transport) {
    ...
    for (int i = 0; i < ibgda_state->n_devs_selected; i++) {
        ...
        status = ibgda_destroy_dci_shared_objects(ibgda_state, device);
        NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                              "ibgda_destroy_dci_shared_objects failed.\n");
    }
    ...
    free(ibgda_state->device_state_cache->cq_h);
    free(ibgda_state->device_state_cache->rc_h);
    free(ibgda_state->device_state_cache);
    ibgda_state->device_state_cache = NULL;
...
static int ibgda_check_nic_mapping_memtypes(nvshmemt_ibgda_state_t *ibgda_state,
    ...
    if (try_gpumem) {
        ...
        if (!ibgda_state->options->IB_DISABLE_DMABUF && ibgda_state->cuda_support_dmabuf) {
            status = ibgda_mobject_nic_map(mobject, device->common_device.context,
                                           IBV_ACCESS_LOCAL_WRITE, true);
            ibgda_state->dmabuf_support_for_control_buffers = (status == 0);
        }

        if (!ibgda_state->dmabuf_support_for_control_buffers) {
            status = ibgda_mobject_nic_map(mobject, device->common_device.context,
                                           IBV_ACCESS_LOCAL_WRITE, false);
            if (status) goto out_try_gpumem;
        }
    ...
    if (try_hostmem) {
        ...
        status =
            ibgda_mobject_nic_map(mobject, device->common_device.context, IBV_ACCESS_LOCAL_WRITE);
        if (status) goto out_try_hostmem;
...
static int ibgda_check_gpu_mapping_nic_uar(struct ibgda_device *device) {
    ...
    status =
        ibgda_alloc_and_map_qp_uar(device->common_device.context, IBGDA_NIC_HANDLER_GPU, &mobject);
    if (status) {
        NVSHMEMI_WARN_PRINT(
...
int nvshmemt_init(nvshmem_transport_t *t, struct nvshmemi_cuda_fn_table *table, int api_version) {
    ...
    nvshmemt_ibgda_device_state_cache_t device_state_cache = NULL;
    ...
    ibgda_nic_handler_t nic_handler_request = IBGDA_NIC_HANDLER_AUTO;
    ...
    device_state_cache = (nvshmemt_ibgda_device_state_cache_t)calloc(
        1, sizeof(struct nvshmemt_ibgda_device_state_cache));
    NVSHMEMI_NULL_ERROR_JMP(device_state_cache, status, NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                            "Unable to allocate device state cache for ibgda transport.\n");
...
#ifdef NVSHMEM_USE_MLX5DV
    if (!ibgda_state->options->DISABLE_DATA_DIRECT) {
        if (nvshmemt_mlx5dv_ftable_init(&mlx5dv_handle, &mlx5dv_ftable, ibgda_state->log_level)) {
...
#else
    INFO(ibgda_state->log_level, "directNIC features are disabled");
#endif
    ...
    INFO(ibgda_state->log_level, "NVSHMEM_IBGDA_NIC_HANDLER: %s\n", options->IBGDA_NIC_HANDLER);
    ...
    for (int i = 0; i < num_devices; i++) {
        ...
        if (nic_handler_request == IBGDA_NIC_HANDLER_CPU_GDRCOPY) {
            device->nic_handler = IBGDA_NIC_HANDLER_CPU_GDRCOPY;
        } else if (nic_handler_request == IBGDA_NIC_HANDLER_CPU_HOST_MEMORY) {
            device->nic_handler = IBGDA_NIC_HANDLER_CPU_HOST_MEMORY;
        } else {
            ...
            if (status) {
                ...
                device->nic_handler = IBGDA_NIC_HANDLER_GPU;
            }
        }
        ...
        INFO(ibgda_state->log_level,
             ...
             i, num_devices, name, device->common_device.device_attr.phys_port_cnt);
        int device_used = 0;
    ...
    for (int i = 0; i < transport->n_devices; i++) {
        ...
        if (((struct ibgda_device *)ibgda_state->devices)[ibgda_state->dev_ids[i]].data_direct &&
            !ibgda_state->options->IBGDA_NUM_RC_PER_PE_provided) {
            // Need 4 QPs for achieving bandwidth in data direct device
            ibgda_state->options->IBGDA_NUM_RC_PER_PE = 8;
            INFO(ibgda_state->log_level,
                 "Setting IBGDA_NUM_RC_PER_PE = 8 as data direct device is detected");
        }
    }
    ...
    if (nic_handler == IBGDA_NIC_HANDLER_CPU_GDRCOPY) {
        assert(use_gdrcopy);
        INFO(ibgda_state->log_level, "NIC handler will be CPU with gdrcopy backend.\n");
    } else if (nic_handler == IBGDA_NIC_HANDLER_CPU_HOST_MEMORY) {
        INFO(ibgda_state->log_level, "NIC handler will be CPU with host memory backend.\n");
    } else {
    ...
    ibgda_initialize_cache_state(ibgda_state);
