...
/*
 * Break if someone tries to change the meaning of these
 * enums. They correspond nicely to is_proxy, so we can
 * directly use is_proxy in the critical path.
 */
static_assert(NVSHMEMT_LIBFABRIC_HOST_EP_IDX == 0);
static_assert(NVSHMEMT_LIBFABRIC_PROXY_EP_IDX == 1);
...
int nvshmemt_libfabric_put_signal_completion(nvshmem_transport_t transport,
                                             nvshmemt_libfabric_tx_progress_group_t *ep,
...
static inline nvshmemt_libfabric_tx_progress_group_t* get_tx_progress_group(nvshmemt_libfabric_state_t* state, int is_proxy) {
    nvshmemt_libfabric_tx_progress_group_t* tx_group;
    tx_group = state->eps[is_proxy][state->cur_nic_idx[is_proxy]];
    state->cur_nic_idx[is_proxy] = (state->cur_nic_idx[is_proxy] + 1) % state->num_selected_devs;
    return tx_group;
}

static void nvshmemt_libfabric_put_signal_ack_completion(nvshmemt_libfabric_tx_progress_group_t *ep,
...
static int nvshmemt_libfabric_gdr_process_completion(nvshmem_transport_t transport,
    ...
    if (entry->flags & FI_SEND) {
        state->op_queue[ep->nic_idx]->putToSend(op);
        ...
        op->ep = ep;
        state->op_queue[ep->nic_idx]->putToRecv(op);
    } else {
        NVSHMEMI_ERROR_JMP(status, NVSHMEMX_ERROR_INVALID_VALUE, out,
...
static int nvshmemt_libfabric_single_ep_progress(nvshmem_transport_t transport, nvshmemt_libfabric_tx_progress_group_t *ep, int is_proxy) {
    nvshmemt_libfabric_state_t *state = (nvshmemt_libfabric_state_t *)transport->state;
...
static int nvshmemt_libfabric_auto_progress(nvshmem_transport_t transport, int is_proxy) {
    ...
    for (auto &ep : state->eps[is_proxy]) {
        status = nvshmemt_libfabric_single_ep_progress(transport, ep, is_proxy);
        if (status) return NVSHMEMX_ERROR_INTERNAL;
    }
...
static int nvshmemt_libfabric_slow_progress(nvshmem_transport_t transport, int is_proxy) {
    int status;
    for (int i = 0; i < NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS; i++) {
...
int gdrcopy_amo_ack(nvshmem_transport_t transport, nvshmemt_libfabric_tx_progress_group_t *ep, fi_addr_t dest_addr,
    ...
    do {
        resp_op = (nvshmemt_libfabric_gdr_op_ctx_t *)state->op_queue[ep->nic_idx]->getNextSend();
    ...
    do {
        status = fi_writedata(ep->endpoint, resp_op, 0,
                              fi_mr_desc(state->mr[ep->nic_idx]),
                              imm_data,
                              dest_addr, (uint64_t)state->remote_addr_staged_amo_ack[pe],
                              state->rkey_staged_amo_ack[pe * state->num_selected_devs + ep->nic_idx], &resp_op->ofi_context);
    } while (try_again(transport, &status, &num_retries, is_proxy));
...
int perform_gdrcopy_amo(nvshmem_transport_t transport, nvshmemt_libfabric_gdr_op_ctx_t *op,
    ...
    handle_info = (nvshmemt_libfabric_memhandle_info_t *)nvshmemt_mem_handle_cache_get(
        transport, state->cache, received_op->target_addr);
    ...
    *ptr = new_value;
    ...
    if (received_op->op > NVSHMEMI_AMO_END_OF_NONFETCH) {
        do {
            resp_op = (nvshmemt_libfabric_gdr_op_ctx_t *)state->op_queue[op->ep->nic_idx]->getNextSend();
            status = resp_op == NULL ? -EAGAIN : 0;
        } while (try_again(transport, &status, &num_retries, is_proxy));
        ...
        do {
            status =
                fi_send(op->ep->endpoint, (void *)resp_op, NVSHMEM_STAGED_AMO_WIREDATA_SIZE,
                        fi_mr_desc(state->mr[op->ep->nic_idx]), op->src_addr, &resp_op->ofi_context);
        } while (try_again(transport, &status, &num_retries, is_proxy));
        NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                              "Unable to respond to atomic request.\n");
        op->ep->submitted_ops++;
...
int nvshmemt_libfabric_gdr_process_ack(nvshmem_transport_t transport,
    ...
    handle_info = (nvshmemt_libfabric_memhandle_info_t *)nvshmemt_mem_handle_cache_get(
        transport, state->cache, ret->ret_addr);
    if (!handle_info) {
...
int nvshmemt_libfabric_gdr_process_amos(nvshmem_transport_t transport, int is_proxy) {
    ...
    for (auto & op_queue : state->op_queue) {
        op = (nvshmemt_libfabric_gdr_op_ctx_t *)op_queue->getNextRecv();
        while (op) {
            ...
            status = fi_recv(op->ep->endpoint, (void *)op, NVSHMEM_STAGED_AMO_WIREDATA_SIZE,
                             fi_mr_desc(state->mr[op->ep->nic_idx]), FI_ADDR_UNSPEC, &op->ofi_context);
            if (status) {
...
int nvshmemt_libfabric_put_signal_completion(nvshmem_transport_t transport,
    ...
    if (!iter->second.second) {
        ...
        status = fi_recv(ep->endpoint, op, NVSHMEM_STAGED_AMO_WIREDATA_SIZE,
                         fi_mr_desc(state->mr[ep->nic_idx]), FI_ADDR_UNSPEC, &op->ofi_context);
    }
...
static int nvshmemt_libfabric_quiet(struct nvshmem_transport *tcurr, int pe, int is_proxy) {
    ...
    /* Assume all selected sub-transports have equal progress attributes */
    if (use_staged_atomics == true) {
        if (state->prov_infos[0]->domain_attr->control_progress == FI_PROGRESS_AUTO) {
            for (;;) {
                all_nics_quieted = true;
                for (auto &ep : state->eps[is_proxy]) {
                    completed = fi_cntr_read(ep->counter) + ep->completed_staged_atomics;
                    if (ep->submitted_ops != completed) {
                        all_nics_quieted = false;
                        if (nvshmemt_libfabric_single_ep_progress(tcurr, ep, is_proxy)) {
                            status = NVSHMEMX_ERROR_INTERNAL;
                            break;
                        }
                    }
            ...
            for (;;) {
                ...
                if (nvshmemt_libfabric_slow_progress(tcurr, is_proxy)) {
                    status = NVSHMEMX_ERROR_INTERNAL;
                    break;
                }
            }
...
static int nvshmemt_libfabric_rma_impl(struct nvshmem_transport *tcurr, int pe, rma_verb_t verb,
                                       ...
                                       uint32_t *imm_data, nvshmemt_libfabric_tx_progress_group_t *ep) {
    ...
    if (!ep)
        ep = get_tx_progress_group(state, is_proxy);

    target_ep = pe * state->num_selected_devs * NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS + ep->nic_idx * NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS + is_proxy;

    if (state->provider == NVSHMEMT_LIBFABRIC_PROVIDER_EFA) {
        nvshmemt_libfabric_gdr_op_ctx_t *gdr_ctx;
        do {
            gdr_ctx = (nvshmemt_libfabric_gdr_op_ctx_t *)state->op_queue[ep->nic_idx]->getNextSend();
    ...
    remote_handle = &((nvshmemt_libfabric_mem_handle_t *)remote->handle)->hdls[is_proxy * state->num_selected_devs + ep->nic_idx];
    local_handle = &((nvshmemt_libfabric_mem_handle_t *)local->handle)->hdls[is_proxy * state->num_selected_devs + ep->nic_idx];
    ...
    if (verb.desc == NVSHMEMI_OP_P) {
        ...
        if (state->provider == NVSHMEMT_LIBFABRIC_PROVIDER_EFA) {
            ...
            do {
                ...
                status = fi_write(ep->endpoint, &p_buf->p_op.value, op_size, fi_mr_desc(state->mr[ep->nic_idx]), target_ep,
            ...
            if (state->prov_infos[ep->nic_idx]->domain_attr->mr_mode & FI_MR_VIRT_ADDR)
...
static int nvshmemt_libfabric_rma(struct nvshmem_transport *tcurr, int pe, rma_verb_t verb,
                                  rma_memdesc_t *remote, rma_memdesc_t *local,
                                  rma_bytesdesc_t bytesdesc, int is_proxy) {
    return nvshmemt_libfabric_rma_impl(tcurr, pe, verb, remote, local, bytesdesc, is_proxy, NULL, NULL);
}
...
static int nvshmemt_libfabric_gdr_amo(struct nvshmem_transport *transport, int pe, void *curetptr,
    ...
    do {
        amo = (nvshmemt_libfabric_gdr_op_ctx_t *)state->op_queue[ep->nic_idx]->getNextSend();
    ...
    num_retries = 0;
            do {
                status = fi_send(ep->endpoint, (void *)amo, NVSHMEM_STAGED_AMO_WIREDATA_SIZE, fi_mr_desc(state->mr[ep->nic_idx]),
...
static int nvshmemt_libfabric_amo(struct nvshmem_transport *transport, int pe, void *curetptr,
    ...
    remote_handle =
        &((nvshmemt_libfabric_mem_handle_t *)remote->remote_memdesc.handle)->hdls[is_proxy * state->num_selected_devs + ep->nic_idx];
    if (verb.desc > NVSHMEMI_AMO_END_OF_NONFETCH) {
        local_handle = &((nvshmemt_libfabric_mem_handle_t *)remote->ret_handle)->hdls[is_proxy * state->num_selected_devs + ep->nic_idx];
    }
...
static int nvshmemt_libfabric_gdr_signal(struct nvshmem_transport *transport, int pe,
    ...
    do {
        context = (nvshmemt_libfabric_gdr_op_ctx_t *) state->op_queue[ep->nic_idx]->getNextSend();
    ...
    num_retries = 0;
    do {
        status = fi_send(ep->endpoint, (void *)signal, sizeof(nvshmemt_libfabric_gdr_signal_op_t), fi_mr_desc(state->mr[ep->nic_idx]),
...
int nvshmemt_put_signal_unordered(struct nvshmem_transport *tcurr, int pe, rma_verb_t write_verb,
    ...
    int status;
    uint32_t sequence_count = 0;
    nvshmemt_libfabric_tx_progress_group_t *ep = get_tx_progress_group(state, is_proxy);
    ...
    if (unlikely(status)) {
        NVSHMEMI_ERROR_PRINT(
            "Error in nvshmemt_put_signal_unordered while waiting for category\n");
        goto out;
    }
    ...
    for (size_t i = 0; i < write_remote.size(); i++) {
        status =
            nvshmemt_libfabric_rma_impl(tcurr, pe, write_verb, &write_remote[i], &write_local[i],
                                        write_bytes_desc[i], is_proxy, &sequence_count, ep);
        if (unlikely(status)) {
            NVSHMEMI_ERROR_PRINT(
    ...
    status = nvshmemt_libfabric_gdr_signal(tcurr, pe, NULL, sig_verb, sig_target, sig_bytes_desc,
                                           is_proxy, sequence_count, (uint16_t) write_remote.size(), ep);
out:
    if (status) {
        NVSHMEMI_ERROR_PRINT(
...
static int nvshmemt_libfabric_enforce_cst(struct nvshmem_transport *tcurr) {
    nvshmemt_libfabric_state_t *state = (nvshmemt_libfabric_state_t *)tcurr->state;
    /* Only enforce cst on proxy endpoints */
    nvshmemt_libfabric_tx_progress_group_t *ep = get_tx_progress_group(state, 1);
    uint64_t num_retries = 0;
    int status = 0;
...
skip:
    ...
    do {
        ...
        void *desc = state->local_mr_desc[NVSHMEMT_LIBFABRIC_PROXY_EP_IDX];
        uint64_t flags = 0;
    ...
    memset(&r_iov, 0, sizeof(struct fi_rma_iov));
        ...
        l_iov.iov_base = state->local_mem_ptr;
        l_iov.iov_len = 8;

        r_iov.addr = 0;  // Zero offset
        r_iov.len = 8;
        r_iov.key = state->local_mr_key[NVSHMEMT_LIBFABRIC_PROXY_EP_IDX];
    ...
    /* Assume all info's share same caps */
    if (state->prov_infos[0]->caps & FI_FENCE) flags |= FI_FENCE;

        status =
            fi_readmsg(ep->endpoint, &msg, flags);
...
static int nvshmemt_libfabric_release_mem_handle(nvshmem_mem_handle_t *mem_handle,
    ...
    if (state->provider == NVSHMEMT_LIBFABRIC_PROVIDER_EFA) {
        nvshmemt_libfabric_memhandle_info_t *handle_info;
        handle_info = (nvshmemt_libfabric_memhandle_info_t *)nvshmemt_mem_handle_cache_get(
            t, state->cache, fabric_handle->buf);
        if (handle_info != NULL) {
#ifdef NVSHMEM_USE_GDRCOPY
    ...
    /* Assume all domain's claim same status for FI_MR_ENDPOINT */
    if (state->prov_infos[0]->domain_attr->mr_mode & FI_MR_ENDPOINT)
        max_reg = NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS * state->num_selected_devs;
    else
        max_reg = state->num_selected_devs;

    /*
     * MR's are stored in the following pattern:
     * i=0 (HOST), j=0 (dev0): hdl_idx = 0*2 + 0 = 0  → dev_0_host
     * i=0 (HOST), j=1 (dev1): hdl_idx = 0*2 + 1 = 1  → dev_1_host
     * i=1 (PROXY), j=0 (dev0): hdl_idx = 1*2 + 0 = 2 → dev_0_proxy
     * i=1 (PROXY), j=1 (dev1): hdl_idx = 1*2 + 1 = 3 → dev_1_proxy
     *
     * This loop iterates through i=0 case for non FI_MR_ENDPOINT,
     * and all registrations for FI_MR_ENDPOINT.
     */
    for (int i = 0; i < max_reg; i++) {
        if (state->local_mr[i] == fabric_handle->hdls[i].mr)
            state->local_mr[i] = NULL;
...
static int nvshmemt_libfabric_get_mem_handle(nvshmem_mem_handle_t *mem_handle, void *buf,
    ...
    status = CUPFN(state->table, cuCtxGetDevice(&gpu_device_id));
    ...
    // TODO NEED TO DEAL WITH fabric_handle->buf
    fabric_handle->buf = buf;
    /* Assume all domain's claim same status for FI_MR_ENDPOINT */
    if (state->prov_infos[0]->domain_attr->mr_mode & FI_MR_ENDPOINT) {
        for (int i = 0; i < NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS; i++) {
            for (size_t j = 0; j < state->domains.size(); j++) {
                int hdl_idx = i * state->num_selected_devs + j;
                status =
                    fi_mr_regattr(state->domains[j], &mr_attr, 0, &fabric_handle->hdls[hdl_idx].mr);
                NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                                      "Error registering memory region: %s\n",
                                      fi_strerror(status * -1));

                status =
                    fi_mr_bind(fabric_handle->hdls[hdl_idx].mr, &state->eps[i][j]->endpoint->fid, 0);

                NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                                      "Error binding MR to EP %d: %s\n", i, fi_strerror(status * -1));

                status = fi_mr_enable(fabric_handle->hdls[hdl_idx].mr);
                NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out, "Error enabling MR: %s\n",
                                      fi_strerror(status * -1));

                fabric_handle->hdls[hdl_idx].key = fi_mr_key(fabric_handle->hdls[hdl_idx].mr);
                fabric_handle->hdls[hdl_idx].local_desc = fi_mr_desc(fabric_handle->hdls[hdl_idx].mr);
            }
        }
    } else {
        // TODO deal with fabric_handle's
        for (size_t j = 0; j < state->domains.size(); j++) {
            struct fid_mr *mr;

            status = fi_mr_regattr(state->domains[j], &mr_attr, 0, &mr);
            NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                                  "Error registering memory region: %s\n", fi_strerror(status * -1));

            for (int i = 0; i < NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS; i++) {
                int hdl_idx = i * state->num_selected_devs + j;
                fabric_handle->hdls[hdl_idx].mr = mr;
                fabric_handle->hdls[hdl_idx].key = fi_mr_key(mr);
                fabric_handle->hdls[hdl_idx].local_desc = fi_mr_desc(mr);
            }
        }
    }

    if (!local_only && state->provider == NVSHMEMT_LIBFABRIC_PROVIDER_EFA) {
        if (state->cache == NULL) {
            status = nvshmemt_mem_handle_cache_init(t, &state->cache);
        ...
        if (!is_host) {
            ...
            if (use_gdrcopy) {
                ...
                curr_ptr = buf;
                do {
                    status = nvshmemt_mem_handle_cache_add(t, state->cache, curr_ptr,
                                                           (void *)handle_info);
    ...
    if (state->local_mr[0] == NULL && !local_only) {
        for (int i = 0; i < NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS; i++) {
            state->local_mr[i] = fabric_handle->hdls[i].mr;
            state->local_mr_key[i] = fabric_handle->hdls[i].key;
            state->local_mr_desc[i] = fabric_handle->hdls[i].local_desc;
        }
        state->local_mem_ptr = buf;
    }
...
out:
    if (status) {
        if (handle_info) {
            if (state->cache) {
                nvshmemt_mem_handle_cache_remove(t, state->cache, buf);
            }
            free(handle_info);
        }
    }
...
static int get_pci_path(int dev, char **pci_path, nvshmem_transport_t t) {
    ...
    nic_name = (const char *)state->domain_names[dev].name;
...
static int nvshmemt_libfabric_connect_endpoints(nvshmem_transport_t t, int *selected_dev_ids,
    ...
    if (state->eps[NVSHMEMT_LIBFABRIC_HOST_EP_IDX].size()) {
    ...
    if (state->num_selected_devs > NVSHMEMT_LIBFABRIC_MAX_NIC_PER_PE) {
        state->num_selected_devs = state->num_selected_devs;
        NVSHMEMI_WARN_PRINT("PE selected %d devices, but the libfabric transport only supports a max of %d devices. Continuing using %d devices.\n",
                            state->num_selected_devs, NVSHMEMT_LIBFABRIC_MAX_NIC_PER_PE, NVSHMEMT_LIBFABRIC_MAX_NIC_PER_PE);
    }
    ...
    av_attr.count = NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS * n_pes;
    av_attr.ep_per_node = NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS;
    ...
    for (int dev_idx = 0; dev_idx < state->num_selected_devs; dev_idx++) {
        ...
        if (state->provider == NVSHMEMT_LIBFABRIC_PROVIDER_EFA && strcmp(current_info->fabric_attr->name, "efa-direct"))
            NVSHMEMI_WARN_PRINT("Libfabric transport is using efa fabric instead of efa-direct, "
                                "use libfabric v2.1.0 or newer for improved performance\n");
    }

    /* Allocate out of band AV name exchange buffers */
    local_ep_names = (nvshmemt_libfabric_ep_name_t *)calloc(NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS * state->num_selected_devs,
    ...
    total_num_eps = NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS * n_pes * state->num_selected_devs;
    all_ep_names =
    ...
    for (size_t i = 0; i < state->prov_infos.size(); i++) {
        INFO(state->log_level, "Selected provider %s, fabric %s, nic %s, hmem %s multi-rail %zu/%d\n",
             state->prov_infos[i]->fabric_attr->prov_name, state->prov_infos[i]->fabric_attr->name,
             state->prov_infos[i]->nic->device_attr->name, state->prov_infos[i]->caps & FI_HMEM ? "yes" : "no",
             i+1, num_selected_devs);
        ...
        if (state->provider == NVSHMEMT_LIBFABRIC_PROVIDER_EFA) {
            num_sends = state->prov_infos[i]->tx_attr->size * NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS;
            num_recvs = state->prov_infos[i]->rx_attr->size * NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS;
            elem_size = sizeof(nvshmemt_libfabric_gdr_op_ctx_t);
            num_recvs_per_ep = num_recvs / NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS;
        ...
        for (int j = 0; j < NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS; j++) {
            /*
             * TODO - This is needed because EFA will only bind AVs to EPs on a 1:1 basis.
             * EFA Lifted this requirement in libfabric 2.2.x, we can reduce the number of AVs
             * required if we set libfabric API version to 2.2.
             */
            status = fi_av_open(domain, &av_attr, &address, NULL);
            NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                                  "Failed to allocate address vector: %d: %s\n", status,
                                  fi_strerror(status * -1));
            state->addresses[j].push_back(address);
        }

        /* Initialize nvshmemt_libfabric_tx_progress_group_t resources */
        for (int j = 0; j < NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS; j++) {
            state->eps[j].push_back((nvshmemt_libfabric_tx_progress_group_t *)calloc(1, sizeof(nvshmemt_libfabric_tx_progress_group_t)));
            NVSHMEMI_NULL_ERROR_JMP(state->eps[NVSHMEMT_LIBFABRIC_HOST_EP_IDX][i], status, NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                                    "Unable to alloc libfabric_tx_progress_group struct.\n");
            state->eps[j][i]->nic_idx = i;
            state->eps[j][i]->group_idx = j;

            /* Initialize per-endpoint proxy_put_signal_comp_map */
            state->eps[j][i]->proxy_put_signal_comp_map =
                new std::unordered_map<uint64_t, std::pair<nvshmemt_libfabric_gdr_op_ctx_t *, int>>();

            state->eps[j][i]->put_signal_seq_counter.reset();
            state->eps[j][i]->completed_staged_atomics = 0;

            status = fi_cq_open(domain, &cq_attr, &state->eps[j][i]->cq, NULL);
            NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                              "Unable to open completion queue for endpoint: %d: %s\n", status,
                              fi_strerror(status * -1));

            status = fi_cntr_open(domain, &cntr_attr, &state->eps[j][i]->counter, NULL);
            NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                                  "Unable to open counter for endpoint: %d: %s\n", status,
                                  fi_strerror(status * -1));

            status = fi_endpoint(domain, state->prov_infos[i], &state->eps[j][i]->endpoint, NULL);
            NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
            ...
            if (state->provider == NVSHMEMT_LIBFABRIC_PROVIDER_EFA) {
                ...
                status = fi_setopt(&state->eps[j][i]->endpoint->fid, FI_OPT_ENDPOINT,
            ...
            status = fi_ep_bind(state->eps[j][i]->endpoint, &state->addresses[j][i]->fid, 0);
            NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
            ...
            status = fi_ep_bind(state->eps[j][i]->endpoint, &state->eps[j][i]->cq->fid, flags);
            NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
            ...
            if (use_staged_atomics)
                flags |= FI_SEND;
            status = fi_ep_bind(state->eps[j][i]->endpoint, &state->eps[j][i]->counter->fid, flags);
            NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                                  "Unable to bind endpoint to completion counter: %d: %s\n", status,
                                  fi_strerror(status * -1));

            status = fi_enable(state->eps[j][i]->endpoint);
            NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                                  "Unable to enable endpoint: %d: %s\n", status,
                                  fi_strerror(status * -1));

            if (state->provider == NVSHMEMT_LIBFABRIC_PROVIDER_EFA) {
                for (size_t k = 0; k < num_recvs_per_ep; k++) {
                    nvshmemt_libfabric_gdr_op_ctx_t *op;
                    op = (nvshmemt_libfabric_gdr_op_ctx_t *)state->recv_buf[i];
                    op = op + ((num_recvs_per_ep * j) + k);
                    assert(op != NULL);
                    status = fi_recv(state->eps[j][i]->endpoint, op, NVSHMEM_STAGED_AMO_WIREDATA_SIZE,
            ...
            status = fi_getname(&state->eps[j][i]->endpoint->fid, local_ep_names[i * NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS + j].name, &ep_namelen);
            NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
    ...
    status = t->boot_handle->allgather(
        ...
        state->num_selected_devs * NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS * sizeof(nvshmemt_libfabric_ep_name_t),
        t->boot_handle);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                          "Failed to gather endpoint names.\n");

    /* We need to insert one at a time since each buffer is larger than the address. */
    for (int k = 0; k < state->num_selected_devs; k++) {
        for (int j = 0; j < NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS; j++) {
            for (int i = 0; i < total_num_eps; i++) {
                status = fi_av_insert(state->addresses[j][k], &all_ep_names[i], 1, NULL, 0, NULL);
                if (status < 1) {
                    NVSHMEMI_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
    ...
    if (use_staged_atomics) {
        ...
        state->rkey_staged_amo_ack = (uint64_t*)calloc(sizeof(uint64_t), t->n_pes * state->num_selected_devs);
        ...
        for (size_t i = 0; i < state->domains.size(); i++) {
            status = fi_mr_reg(state->domains[i], state->remote_addr_staged_amo_ack[t->my_pe], sizeof(int),
                        FI_REMOTE_WRITE, 0, 0, 0, &mr, NULL);
            NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                                  "Failed to register EFA msg buffer: %d: %s\n", status, fi_strerror(status * -1));
            state->rkey_staged_amo_ack[t->my_pe * state->num_selected_devs + i] = fi_mr_key(mr);
        ...
        status = t->boot_handle->allgather(
            &state->rkey_staged_amo_ack[t->my_pe * state->num_selected_devs], state->rkey_staged_amo_ack,
            sizeof(uint64_t) * state->num_selected_devs, t->boot_handle);
        NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
...
out:
    if (status != 0) {
        ...
        for (int j = 0; j < NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS; j++) {
            for (size_t i = 0; i < state->eps[j].size(); i++) {
                if (state->eps[j][i]->proxy_put_signal_comp_map)
                    delete state->eps[j][i]->proxy_put_signal_comp_map;
                if (state->eps[j][i]->endpoint) {
                    fi_close(&state->eps[j][i]->endpoint->fid);
                    state->eps[j][i]->endpoint = NULL;
                }
                if (state->eps[j][i]->cq) {
                    fi_close(&state->eps[j][i]->cq->fid);
                    state->eps[j][i]->cq = NULL;
                }
                if (state->eps[j][i]->counter) {
                    fi_close(&state->eps[j][i]->counter->fid);
                    state->eps[j][i]->counter = NULL;
                }
                free(state->eps[j][i]);
                state->eps[j][i] = NULL;
            }
...
// TODO REMOVE CLOSE FOR NON-DUP INFO
static int nvshmemt_libfabric_finalize(nvshmem_transport_t transport) {
     ...
     */
    if (state->all_prov_info)
        fi_freeinfo(state->all_prov_info);
    ...
    for (int j = 0; j < NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS; j++) {
        for (size_t i = 0; i < state->eps[j].size(); i++) {
            if (state->eps[j][i]->proxy_put_signal_comp_map)
                delete state->eps[j][i]->proxy_put_signal_comp_map;
            if (state->eps[j][i]->endpoint) {
                status = fi_close(&state->eps[j][i]->endpoint->fid);
                if (status) {
            ...
            if (state->eps[j][i]->cq) {
                status = fi_close(&state->eps[j][i]->cq->fid);
                if (status) {
            ...
            if (state->eps[j][i]->counter) {
                status = fi_close(&state->eps[j][i]->counter->fid);
                if (status) {
    ...
    for (size_t i = 0; i < state->recv_buf.size(); i++)
        free(state->recv_buf[i]);

    for (int j = 0; j < NVSHMEMT_LIBFABRIC_NUM_THREAD_COMPLETION_GROUPS; j++) {
        for (size_t i = 0; i < state->addresses[j].size(); i++) {
            status = fi_close(&state->addresses[j][i]->fid);
            if (status) {
...
static int nvshmemi_libfabric_init_state(nvshmem_transport_t t, nvshmemt_libfabric_state_t *state,
    ...
    struct fid_nic nic;
    struct fi_av_attr av_attr;
    struct fi_info *returned_fabrics, *current_info;
    ...
    for (current_info = returned_fabrics; current_info != NULL;
         current_info = current_info->next) {
...
int nvshmemt_init(nvshmem_transport_t *t, struct nvshmemi_cuda_fn_table *table, int api_version) {
    nvshmemt_libfabric_state_t *state = NULL;
    ...
    state = (nvshmemt_libfabric_state_t *)calloc(1, sizeof(*state));
    NVSHMEMI_NULL_ERROR_JMP(state, status, NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
    ...
    state->max_nic_per_pe = options.LIBFABRIC_MAX_NIC_PER_PE;
    ...
    if (strcmp(options.LIBFABRIC_PROVIDER, "verbs") == 0) {
        ...
        state->provider = NVSHMEMT_LIBFABRIC_PROVIDER_SLINGSHOT;
...
#undef NVSHMEMI_SET_ENV_VAR
    ...
    status = nvshmemi_libfabric_init_state(transport, state, &options);
    if (status) {
